...
// Złożoność czasowa: O(max_a * ln(max_a))
// Złożoność pamięciowa: O(max_a)
// Punkty: 100 (upsolve)
...
// f_div[n] = Najmniejszy czynnik pierwszy liczby n.
int f_div[MAX_A + 5];
vector<int> primes;
...
int32_t main() {
    ...
    for (int i = 1; i <= n; i++) {
        // Przypadek brzegowy, jak a_i występuje conajmniej dwa razy na wejściu
        // to najbliżej będzie oczywiście do pierwszego lub drugiego
        // wystąpienia.
        if (second_occ[a[i]]) {
            if (i == bubble[a[i]])
                cout << second_occ[a[i]] << "\n";
