...
DP dfs_dp(int v, int S, int D) {
    ...
    for (int u : sons[v]) {
        ...
        for (int D_final = D; D_final < (1 << h); D_final++) {
            if ((D_final | D) != D_final) continue;
            for (int D_u = 0; D_u <= D_final; D_u++) {
                if ((D_u | D_final) != D_final) continue;
